...
export const getOhlcv = defineTool({
  ...
  description: [
    ...
    '',
    'Lookback depth matters as much as interval. The default 100 is fine for',
    'a quick price check, but for any directional trade analysis:',
    '  - swing reads (4h/1d) → fetch at least 300 bars so you see 1-year+',
    '    regime on daily (or multi-week on 4h) instead of cherry-picking a',
    '    recent uptrend out of a larger downtrend.',
    '  - position reads (1d/1w) → 500+ bars to capture multi-year structure.',
    '  - backtests → take the full 1000 max so the metrics carry weight.',
    '',
